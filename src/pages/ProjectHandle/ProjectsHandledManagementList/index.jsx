...
const ProjectsHandledManagementList = () => {
  const { isLoading, managementData, id } = useManagementProjectListHook()
  return (
    <div className={clsx("admin-content")}>
      <div className="d-flex align-items-center justify-content-between flex-wrap gap-5 gap-xxl-4">
        <div className="d-flex align-items-center flex-wrap gap-4 gap-xl-3">
          ...
          <h4 className="mb-0">{managementData?.managementDetail?.company_name}</h4>
          <span>{svgIcons.doubleRightArrowIcon}</span>
      ...
      <div className="w-100 mt-5">
        ...
        <LDTable className="mt-4 mb-5" columns={managementProjectsHandledColumns} data={managementData?.projectList} />
      </div>
    </div>
  );
};
